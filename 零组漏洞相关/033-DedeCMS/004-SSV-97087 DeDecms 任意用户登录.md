# 【开启会员登陆】SSV-97087 DeDecms 任意用户登录
...
在`/member/index.php`中会接收uid和action参数。uid为用户名，进入index.php后会验证Cookie中的用户ID与uid(用户名)并确定用户权限。
...
`$this->M_ID`等于Cookie中的DedUserID，我们继续看看GetCookie函数
